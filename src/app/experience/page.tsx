...
export default function ExperiencePage() {
  const experiences = [
    {
      company: "B2B SaaS企業（現場業務支援 / 清掃・ビルメンテナンス領域）",
      period: "2025年10月 ～ 現在",
      project: "現場業務支援SaaSの開発・運用（プロジェクトリード兼フルスタック）",
      description:
        "現場業務（清掃・ビルメンテナンス領域）の受発注・業務管理を扱うB2B SaaSの開発・運用に従事。企画〜要件整理〜進行管理のプロジェクトリードと、フロント／バックエンドの実装を兼務。複数プロジェクトを並行リードし、KPI・工数削減で効果を定義しながら他部署横断の合意形成も担当。",
      role: "プロジェクトリード兼フルスタックエンジニア",
      technologies: [
        "Next.js 15",
        "React 18",
        "TypeScript",
      ...
      responsibilities: [
        "新旧2系統の業務報告画面を統一する完全移行プロジェクトをリード（二重運用による保守コスト・サポート負担を解消）",
        "業務報告帳票（Excel／PDF）の自動改行・レイアウト品質を改善し、日本語として自然な折り返しロジックに刷新＋見切れ防止の回帰テストを整備",
        "問い合わせ導線をメッセージング経由から構造化フォームへ移行し、外部カスタマーサポート基盤へ整形送信（他部署の対応工数を月約60時間削減見込み）",
        "外部メッセージングプラットフォームのOAuthセルフ連携をフロント／バック両面で設計・実装（社内対応工数を月約15時間削減見込み）",
    ...
    {
      company: "有限会社カネマサ",
      period: "2013年07月 ～ 2023年03月",
      project: "リフォーム・ハウスクリーニング事業",
      description:
        "マンション・アパートの原状回復やリフォーム、ハウスクリーニング、空室管理、物件内覧案内、賃貸契約書作成など幅広い業務に従事。施工管理・顧客対応を中心に、現場品質・原価・在庫管理の業務改善を推進。",
      role: "施工管理・顧客対応",
      technologies: ["Excel", "在庫管理システム", "業務改善"],
      responsibilities: [
        "賃貸物件の原状回復・ハウスクリーニング",
        "小規模リフォームの計画・実施",
        "施工管理（工程・品質・安全の統合管理）",
